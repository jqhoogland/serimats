...
def epsilon(learner: "Learner") -> t.Tensor:
    ...
    norm = t.zeros(1, device=learner.device)

    if (
        learner.weight_initializer.initial_weights is None
        or learner.baseline.weight_initializer.initial_weights is None
    ):
        raise ValueError("Initial weights not set")

    for p1, p2 in zip(
        learner.weight_initializer.initial_weights,
        learner.baseline.weight_initializer.initial_weights,
    ):
...
def d_w_from_baseline(learner: "Learner") -> t.Tensor:
    ...
    """
    return learner.lp_distance(learner.baseline)


@add_latex(
    r"\widehat{d_W}(w^{(t)}, w_\mathrm{ref}^{(t)})", r"d_W / w_\mathrm{ref}^{(t)}"
)
...
@add_latex(
    r"C_\mathrm{ref}(t)",
    r"\frac{1}{D}\mathbf{w}^{(t)} \cdot \mathbf{w}_\mathrm{ref}^{(t)}",
)
def w_corr_with_baseline(learner: "Learner") -> t.Tensor:
    ...
    """
    return learner.model @ learner.baseline.model


@add_latex(
    r"S_C(w^{(t)}, w_\mathrm{ref}^{(t)})",
    r"\frac{|\mathbf{w}^{(t)} \cdot \mathbf{w}_\mathrm{ref}^{(t)}|}{|w^{(t)}| |w_\mathrm{ref}^{(t)}|}",
)
def cos_sim_from_baseline(learner: "Learner") -> t.Tensor:
    ...
    """
    return learner.cosine_similarity(learner.baseline)
...
def d_w_from_init(learner: "Learner") -> t.Tensor:
    ...
    """
    return learner.lp_distance(learner.weight_initializer.initial_weights)
...
@add_latex(
    r"S_C(w^{(t)}, w^{(0)})",
    r"\frac{|\mathbf{w}^{(t)} \cdot \mathbf{w}^{(0)}|}{|w^{(t)}| |w^{(0)}|}",
)
def cos_sim_from_init(learner: "Learner") -> t.Tensor:
    ...
    """
    return learner.cosine_similarity(learner.weight_initializer.initial_weights)
...
def _weight_space_metrics(learner: "Learner", **_) -> Dict[str, t.Tensor]:
    ...
    metrics["d_w_from_baseline_normed"] = normalize_wrt_epsilon(
        metrics["d_w_from_baseline"], learner
    )
    metrics["d_w_from_init_normed"] = normalize_wrt_init(
        metrics["d_w_from_init"], learner
    )
...
def _function_space_metrics(
    ...
    baseline_preds = {
        baseline: baseline_outputs[baseline].argmax(dim=1, keepdim=False)
    ...
    for i, learner in enumerate(learners):
        ...
        ).item()  # TODO: Custom loss to allow for multi-target
...
def metrics(
    ...
    **_,
    ...
    for name, dl in dls:
        for i, learner_metrics in enumerate(
            function_space_metrics(learners, dl, loss_fn=loss_fn, **_)
        ):
...
def detensorize_metrics(metrics: List[Dict[str, t.Tensor]]) -> List[Dict[str, float]]:
    return [
        {k: v.item() for k, v in learner_metrics.items()} for learner_metrics in metrics
    ]
...
class Metrics:
    ...
    def measure(
        self, learners: List["Learner"], loss_fn=F.nll_loss, **kwargs
    ) -> List[Dict[str, float]]:
        return detensorize_metrics(
            metrics(
                learners,
